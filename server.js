...
const serviceAccountKey = JSON.parse(
  fs.readFileSync(
    "./react-js-blog-website-yt-19cd0-firebase-adminsdk-fbsvc-78d423a6b4.json",
    "utf-8"
  )
);
...
server.post("/signup", (req, res)=>{
    ...
    if(fullname.length<3){
        return res.status(403).json({
            "error": "Fullname must be atleast three letter long"
        })
    }
    ...
    if(!passwordRegex.test(password)){
     return res.status(403).json({
        "error": "Password should be 6-20 character long with a numeric, 1 lowercase and 1 uppercase letters"
     })
    }
    ...
    .then((user)=>{
       ...
       if(!user.google_auth){
          bcrypt.compare(password, user.personal_info.password, (err, result)=>{
            if(err){
                return res.status(403).json({"error": "Error occured while comparing login credentials please try again"})
            }
            if(!result){
        ...
        return res.status(403).json({"error": "Please login using google authentication method"})
       }
...
server.post("/google-auth", async (req, res) => {
    ...
    getAuth()
    .verifyIdToken(access_token).then(async (decodeUser) => {
      let { email, name, picture } = decodeUser;
      picture=picture.replace("s96-c", "s384-c");//s400
        ...
        if(user){
            if(!user.google_auth){
                return res.status(403).json({ "error": "Email already registered with us, please use password to login" });
            }
        }
        else{
            let username=await generateUsername(email);
        ...
        return res.status(500).json({ "error": "Failed to verify google account try with a different account" });
    })
})
...
server.post("/change-password", verifyJWT, (req, res)=>{
    ...
    if(!passwordRegex.test(currentPassword) || !passwordRegex.test(newPassword)){
            return res.status(403).json({ error :"password should be 6 to 20 character long with a numeric, one lowercase and one uppercase letters " });
    }
    User.findOne({_id: req.user}).then((user)=>{
        if(user.google_auth){
            return res.status(403).json({error : " You can not change password because you logged in through Google"})
        }

        bcrypt.compare(currentPassword, user.personal_info.password, (err, result)=>{
            if(err){
               return res.status(500).json({error : "Some error occured while changing the password, please try again letter"})
            }
            if(!result){
            ...
            bcrypt.hash(newPassword, 10, (err, hashed_password)=>{
               ...
               .catch(err =>{
                 return res.status(500).json({error : "Some error occured while saving the new password, please try again letter"})
                })
            })
        })
    }).catch(err=>{
...
server.post("/search-users", (req, res)=>{
    let {query}=req.body;
    User.find({"personal_info.username": new RegExp(query, 'i')}).limit(50).select("personal_info.fullname personal_info.username personal_info.profile_img -_id").then(
        users=>{
...
server.post("/get-blog", (req, res)=>{
    ...
    Blog.findOneAndUpdate({blog_id}, {$inc: {"activity.total_reads": incrementVal}}).populate("author", "personal_info.fullname personal_info.username personal_info.profile_img").select("title des content banner activity publishedAt blog_id tags").then(blog=>{
...
server.post("/get-replies", async (req, res) => {
  try {
    ...
    console.log("Incoming reply request:", _id);

    const doc = await Comment.findById(_id).populate({
      path: "children",
      options: {
        limit: maxLimit,
        skip: skip,
...
const deleteComments=(_id)=>{
    Comment.findOneAndDelete({_id}).then(comment=>{
      ...
      Blog.findOneAndUpdate({_id: comment.blog_id}, {$pull: {comments: _id }, $inc: { "activity.total_comments": -1}, "activity.total_parent_comments": comment.parent ? 0 :1}).then(blog=>{
...
server.post("/delete-comment",verifyJWT, (req, res)=>{
   ...
   Comment.findOne({_id}).then(comment=>{
       ...
       if(user_id == comment.commented_by || user_id == comment.blog_author){
         ...
         return res.status(403).json({error: "Yoc can not delete this comment"})
       }
   })
...
server.post("/notifications", verifyJWT, (req, res)=>{
    ...
    Notification.find(findQuery).skip(skipDocs).limit(maxLimit).populate("blog", "title blog_id").populate("user", "personal_info.fullname personal_info.username personal_info.profile_img").populate("comment", "comment").populate("replied_on_comment", "comment").populate("reply", "comment").sort({createdAt: -1}).select("createdAt type seen reply").then(notifications=>{
        Notification.updateMany(findQuery, {seen: true}).skip(skipDocs).limit(maxLimit).then(()=>console.log("Notification seen"))
